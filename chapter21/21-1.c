...
int size[] = {2, 3, 5, 7, 9}; // 品物の大きさ
int value[] = {2, 4, 7, 11, 14}; // 品物の価値
#define N (sizeof(size)/sizeof(size[0])) // 品物の種類の数
#define MAX_M 200 // ナップザックの大きさの上限
...
int main(void) {
    ...
    int choice[MAX_M]; // 最後に選んだ品物
    int repack_total; // 品物iを入れたときの価値の合計

    if (argc != 2) {
        fprintf(stderr, "usage: knapsack m\n");
        fprintf(stderr, "ここで、m はナップザックの大きさ\n");
        exit(1);
    }
